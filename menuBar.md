...
| Ctrl+Comma | Settings | 设置中心 | 打开设置中心`Comma`即`逗号`
 |
> Settings的内容会在另一篇文章-[初识Atom —— 设置][2]中进行详细介绍。
...
> Atom会首先加载核心功能的快捷键，之后才是插件包中用户定义的快捷键，当然想要解决快捷键冲突，可以编辑`File-Keymap...`来删除及重定义快捷键，keymap文件中用户定义的快捷键优先级是最高的。具体设置方法可看[初识Atom —— keymap快捷键设置][7]。
...
> 可能大家注意到，在前文中提到打开设置中心快捷键是`Ctrl+,`,这里怎么会是`Ctrl+F1`,这是因为我对这个命令的快捷键通过`keymap`文件进行了修改，我可以通过`ctrl+f1`来打开设置中心，具体设置方法可看[初识Atom —— 设置][2]。
